...
func PreviewTemp(c *gin.Context) {
	...
	if err != nil {
		global.SlotsLog.Error("预览失败!", zap.Any("err", err))
...
func CreateTemp(c *gin.Context) {
	...
	if a.AutoCreateApiToSql {
		if err := service.AutoCreateApi(&a); err != nil {
			global.SlotsLog.Error("自动化创建失败!请自行清空垃圾数据!", zap.Any("err", err))
...
func GetTables(c *gin.Context) {
	dbName := c.DefaultQuery("dbName", global.SlotsConfig.Mysql.Dbname)
	err, tables := service.GetTables(dbName)
	if err != nil {
		global.SlotsLog.Error("查询table失败!", zap.Any("err", err))
