...
func ExportExcel(c *gin.Context) {
	...
	filePath := global.SlotsConfig.Excel.Dir + excelInfo.FileName
	err := service.ParseInfoList2Excel(excelInfo.InfoList, filePath)
	if err != nil {
		global.SlotsLog.Error("转换Excel失败!", zap.Any("err", err))
...
func ImportExcel(c *gin.Context) {
	...
	_ = c.SaveUploadedFile(header, global.SlotsConfig.Excel.Dir+"ExcelImport.xlsx")
...
func LoadExcel(c *gin.Context) {
	...
	if err != nil {
		global.SlotsLog.Error("加载数据失败", zap.Any("err", err))
...
func DownloadTemplate(c *gin.Context) {
	...
	filePath := global.SlotsConfig.Excel.Dir + fileName
	ok, err := utils.PathExists(filePath)
	if !ok || err != nil {
		global.SlotsLog.Error("文件不存在", zap.Any("err", err))
