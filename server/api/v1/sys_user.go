...
func Login(c *gin.Context) {
	...
	if store.Verify(L.CaptchaId, L.Captcha, true) {
		...
		if err, user := service.Login(U); err != nil {
			global.TraceLog.Error("登陆失败! 用户名不存在或者密码错误", zap.Any("err", err))
...
func tokenNext(c *gin.Context, user model.SysUser) {
	j := &middleware.JWT{SigningKey: []byte(global.SlotsConfig.JWT.SigningKey)} // 唯一签名
	claims := request.CustomClaims{
		...
		BufferTime:  global.SlotsConfig.JWT.BufferTime, // 缓冲时间1天 缓冲时间内会获得新的token刷新令牌 此时一个用户会存在两个有效令牌 但是前端只留一个 另一个会丢失
		StandardClaims: jwt.StandardClaims{
			NotBefore: time.Now().Unix() - 1000,                               // 签名生效时间
			ExpiresAt: time.Now().Unix() + global.SlotsConfig.JWT.ExpiresTime, // 过期时间 7天  配置文件
			Issuer:    "qmPlus",                                               // 签名的发行者
		},
	}
	token, err := j.CreateToken(claims)
	if err != nil {
		global.TraceLog.Error("获取token失败", zap.Any("err", err))
	...
	if err, jwtStr := service.GetRedisJWT(user.Username); err == redis.Nil {
		if err := service.SetRedisJWT(token, user.Username); err != nil {
			global.TraceLog.Error("设置登录状态失败", zap.Any("err", err))
...
func Register(c *gin.Context) {
	...
	if err != nil {
		global.TraceLog.Error("注册失败", zap.Any("err", err))
...
func ChangePassword(c *gin.Context) {
	...
	if err, _ := service.ChangePassword(U, user.NewPassword); err != nil {
		global.TraceLog.Error("修改失败", zap.Any("err", err))
...
func DeleteUser(c *gin.Context) {
	...
	if err := service.DeleteUser(reqId.ID); err != nil {
		global.TraceLog.Error("删除失败!", zap.Any("err", err))
...
func SetUserInfo(c *gin.Context) {
	...
	if err, ReqUser := service.SetUserInfo(user); err != nil {
		global.TraceLog.Error("设置失败", zap.Any("err", err))
...
func getUserID(c *gin.Context) uint {
	if claims, exists := c.Get("claims"); !exists {
		global.TraceLog.Error("从Gin的Context中获取从jwt解析出来的用户ID失败, 请检查路由是否使用jwt中间件")
		return 0
	} else {
...
func getUserUuid(c *gin.Context) string {
	if claims, exists := c.Get("claims"); !exists {
		global.TraceLog.Error("从Gin的Context中获取从jwt解析出来的用户UUID失败, 请检查路由是否使用jwt中间件")
		return ""
	} else {
		waitUse := claims.(*request.CustomClaims)
