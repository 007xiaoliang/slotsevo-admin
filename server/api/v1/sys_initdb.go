...
func InitDB(c *gin.Context) {
	if global.SlotsDb != nil {
		global.SlotsLog.Error("非法访问")
		response.FailWithMessage("非法访问", c)
		return
	}
	var dbInfo request.InitDB
	if err := c.ShouldBindJSON(&dbInfo); err != nil {
		global.SlotsLog.Error("参数校验不通过", zap.Any("err", err))
		response.FailWithMessage("参数校验不通过", c)
		return
	}
	if err := service.InitDB(dbInfo); err != nil {
		global.SlotsLog.Error("自动创建数据库失败", zap.Any("err", err))
...
func CheckDB(c *gin.Context) {
	if global.SlotsDb != nil {
		global.SlotsLog.Info("数据库无需初始化")
		response.OkWithDetailed(gin.H{
			"needInit": false,
		}, "数据库无需初始化", c)
		return
	} else {
		global.SlotsLog.Info("前往初始化数据库")
		response.OkWithDetailed(gin.H{
			"needInit": true,
		}, "前往初始化数据库", c)
		return
