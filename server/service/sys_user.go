...
func ChangePassword(u *model.SysUser, newPassword string) (err error, userInter *model.SysUser) {
	var user model.SysUser
	u.Password = utils.MD5V([]byte(u.Password))
	err = global.GvaDb.Where("username = ? AND password = ?", u.Username, u.Password).First(&user).Update("password", utils.MD5V([]byte(newPassword))).Error
	return err, u
}
...
func GetUserInfoList(info request.PageInfo) (err error, list interface{}, total int64) {
	...
	db := global.GvaDb.Model(&model.SysUser{})
...
func SetUserAuthority(uuid uuid.UUID, authorityId string) (err error) {
	err = global.GvaDb.Where("uuid = ?", uuid).First(&model.SysUser{}).Update("authority_id", authorityId).Error
	return err
}
