...
func BreakpointContinue(c *gin.Context) {
	...
	if err != nil {
		global.SlotsLog.Error("接收文件失败!", zap.Any("err", err))
		response.FailWithMessage("接收文件失败", c)
		return
	}
	f, err := FileHeader.Open()
	if err != nil {
		global.SlotsLog.Error("文件读取失败!", zap.Any("err", err))
...
func FindFile(c *gin.Context) {
	...
	err, file := service.FindOrCreateFile(fileMd5, fileName, chunkTotal)
	if err != nil {
		global.SlotsLog.Error("查找失败!", zap.Any("err", err))
...
func BreakpointContinueFinish(c *gin.Context) {
	...
	if err != nil {
		global.SlotsLog.Error("文件创建失败!", zap.Any("err", err))
...
func RemoveChunk(c *gin.Context) {
	...
	if err != nil {
		global.SlotsLog.Error("缓存切片删除失败!", zap.Any("err", err))
